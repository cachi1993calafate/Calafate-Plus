...
// --- LÓGICA DE ICONOS CON DIBUJOS (EMOJIS) ---
const getBizIcon = (category: string) => {
  let color = "#3b82f6"; 
  let iconHtml = "📍";

  if (category === "Gastronomía") { color = "#ef4444"; iconHtml = "🍴"; }
  else if (category === "Panadería") { color = "#f59e0b"; iconHtml = "🥖"; }
  else if (category === "Kiosco") { color = "#facc15"; iconHtml = "🛒"; } // Amarillo para Kioscos
  else if (category === "Láser" || category === "Servicios") { color = "#06b6d4"; iconHtml = "🛠️"; }
  else if (category === "Emprendedores") { color = "#8b5cf6"; iconHtml = "🚀"; }
  else if (category === "Regalos") { color = "#ec4899"; iconHtml = "🎁"; }
  else if (category === "Excursiones") { color = "#10b981"; iconHtml = "🏔️"; }
  else { color = "#64748b"; iconHtml = "📦"; } // Varios / Otros
...
export default function CalafatePlus() {
  ...
  const CATEGORIES = ["Todos", "Gastronomía", "Panadería", "Kiosco", "Láser", "Emprendedores", "Regalos", "Excursiones", "Varios"];
      ...
      <main style={{ padding: "20px" }}>
        {filteredBiz.map(biz => (
          <div key={biz.id} style={{ background: "#0a1929", borderRadius: "25px", marginBottom: "25px", padding: "20px", border: "1px solid #1e293b", position: "relative" }}>
            <div style={{ position: "absolute", top: "20px", right: "20px", background: "#ef4444", color: "#fff", width: "55px", height: "55px", borderRadius: "50%", display: "flex", justifyContent: "center", alignItems: "center", fontWeight: "900", fontSize: "12px", textAlign: "center", lineHeight: "1.1", zIndex: 10 }}>{biz.discount_short || "10% OFF"}</div>
