...
import { Download, Camera, LogOut, Settings, Trash2, Edit2, X, Bell, MessageCircle, MapPin } from "lucide-react";
...
export default function CalafatePlus() {
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      #r-scanner_camera_permission_button { background: #3b82f6; color: white; border: none; padding: 10px 20px; borderRadius: 10px; fontWeight: bold; }
    ...
    <div style={{ minHeight: "100vh", background: "#010b14", color: "#fff", fontFamily: 'sans-serif' }}>
      ...
      {view === "scanner" ? (
        ...
        <div style={{ padding: "20px" }}>
          <h2 style={{ color: "#fbbf24", fontWeight: "900", marginBottom: "20px" }}>CONTROL TOTAL</h2>
          <div style={{ background: "#0f172a", padding: "20px", borderRadius: "20px", marginBottom: "30px", border: "1px solid #3b82f6" }}>
            <div style={{ display: "grid", gap: "12px" }}>
              ...
              <input value={newBiz.offer_es} placeholder="Promoción" onChange={e => setNewBiz({...newBiz, offer_es: e.target.value})} style={{ padding: "14px", borderRadius: "12px", background: "#010b14", border: "1px solid #1e293b", color: "#fff" }} />
              <input value={newBiz.phone} placeholder="WhatsApp (Ej: 2902404040)" onChange={e => setNewBiz({...newBiz, phone: e.target.value})} style={{ padding: "14px", borderRadius: "12px", background: "#010b14", border: "1px solid #1e293b", color: "#fff" }} />
          ...
          <div className="table-container" style={{ overflowX: "auto", background: "#0a1929", borderRadius: "20px", border: "1px solid #1e293b", padding: "10px" }}>
            <table style={{ width: "100%", minWidth: "400px" }}>
              <thead><tr style={{ color: "#64748b", fontSize: "11px" }}><th align="left">NEGOCIO</th><th align="center">W/M/Q</th><th align="right">ACCIONES</th></tr></thead>
              <tbody>
                {businesses.map(biz => {
                  ...
                  return (
                    <tr key={biz.id} style={{ borderBottom: "1px solid #1e293b" }}>
                      <td style={{ padding: "12px 5px" }}>
                        ...
                        <div style={{ fontSize: "10px", color: days <= 0 ? "#ef4444" : "#22c55e" }}>{days <= 0 ? 'Vencido' : `${days} d.`}</div>
                      </td>
                      ...
                      <td align="right" style={{ display: "flex", gap: "8px", justifyContent: "flex-end", padding: "12px 0" }}>
                        <Bell size={18} color="#fbbf24" onClick={() => { const msg = `Hola ${biz.name}! Te escribimos de Calafate Plus.`; window.open(`https://wa.me/549${biz.phone}?text=${encodeURIComponent(msg)}`); }} style={{cursor:"pointer"}}/>
        ...
        <>
          ...
          <div style={{ margin: "20px", background: "linear-gradient(135deg, #1e293b 0%, #0a1120 100%)", borderRadius: "25px", padding: "25px", border: "2px solid #3b82f6", textAlign: "center" }}>
            <h2 style={{ margin: "0 0 15px 0", fontSize: "20px", fontWeight: "900", color: "#fff" }}>¿Querés que tu negocio<br/>aparezca acá?</h2>
            <button onClick={() => window.open('https://wa.me/5492902404040')} style={{ background: "#22c55e", color: "#fff", border: "none", padding: "12px 30px", borderRadius: "15px", fontWeight: "900", fontSize: "16px", display: "flex", alignItems: "center", gap: "10px", margin: "0 auto" }}>
            ...
            {publicBiz.map(biz => (
              <div key={biz.id} style={{ background: "#0a1929", borderRadius: "25px", marginBottom: "25px", padding: "20px", border: "1px solid #1e293b", position: "relative" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                  ...
                  <button onClick={() => {trackClick(biz.id, "clicks_wa", biz.clicks_wa); window.open(`https://wa.me/549${biz.phone}`);}} style={{ background: "none", border: "1.5px solid #22c55e", color: "#22c55e", padding: "12px", borderRadius: "15px", fontWeight: "bold", fontSize: "14px", display:"flex", alignItems:"center", justifyContent:"center", gap:"5px" }}><MessageCircle size={18}/> WA</button>
                </div>
              </div>
            ))}
          </main>
